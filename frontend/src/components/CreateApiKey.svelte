    ...
    function handleSubmit(event) {
        ...
        if (keyName === '') {
            notifications.danger(
                $_('pages.userProfile.apiKeys.fields.name.invalid'),
            )
        ...
        xfetch(`/api/user/${$user.id}/apikey`, { body })
            .then(res => res.json())
            .then(function(apk) {
                handleApiKeyCreate()
                apiKey = apk.apiKey
            ...
            .catch(function(error) {
                notifications.danger(
                    $_('pages.userProfile.apiKeys.createFailed'),
                )
...
<Modal closeModal="{toggleCreateApiKey}">
        ...
        <form on:submit="{handleSubmit}" name="createApiKey">
            <div class="mb-4">
                <label class="block text-sm font-bold mb-2" for="keyName">
                    {$_('pages.userProfile.apiKeys.fields.name.label')}
                </label>
                <input
                    ...
                    name="keyName"
                    bind:value="{keyName}"
                    placeholder="{$_('pages.userProfile.apiKeys.fields.name.placeholder')}"
                    required />
            </div>
            <div class="text-right">
                <div>
                    <SolidButton type="submit">
                        {$_('pages.userProfile.apiKeys.fields.submitButton')}
                    </SolidButton>
                </div>
            </div>
        </form>
    {:else}
        <div class="mb-4">
            <p class="mb-3 mt-3">
                {@html $_('pages.userProfile.apiKeys.createSuccess', {
                    values: {
            ...
            <p>{$_('pages.userProfile.apiKeys.storeWarning')}</p>
        </div>
        <div class="text-right">
            <div>
                <SolidButton onClick="{toggleCreateApiKey}">
                    {$_('pages.userProfile.apiKeys.fields.closeButton')}
                </SolidButton>
            </div>
        </div>
    {/if}
