...
CREATE OR REPLACE PROCEDURE move_story(storyId UUID, goalId UUID, columnId UUID)
LANGUAGE plpgsql AS $$
BEGIN
    -- UPDATE storyboard_story SET goal_id = goalId, column_id = columnId, updated_date = NOW() WHERE id = storyId;
END;
$$;
